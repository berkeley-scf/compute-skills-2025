...
Using the [official "The Python Standard Library" reference](https://docs.python.org/3/library/index.html), figure out how to compute:
...
Create a list of numbers, called `x1`. Reverse the order of the items in the list
    ...
    method. How does using the method differ from slicing? Hint: you can type `x1.` followed by the `<Tab>` key in a Notebook or at the IPython prompt to find the various methods that can be applied to a list.
...
- Figure out some different ways of combining your list of numbers with a list of strings to create a single list of mixed type elements. (Hint: you can type `x1.` followed by the `<Tab>` key in a Notebook or at the IPython prompt to find the various methods that can be applied to a list.)
